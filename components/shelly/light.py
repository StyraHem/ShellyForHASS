...
EFFECT_ONOFF_C = { 'name':"On/Off gradual (Color)", 'mode':'color', 'effect':5 }
...
class ShellyRGB(ShellyDevice, Light):
    ...
    def supported_features(self):
        ...
        features =  SUPPORT_SHELLYRGB_WHITE if self._mode=='white' else SUPPORT_SHELLYRGB_COLOR
    ...
    def turn_on(self, **kwargs):
        ...
        if ATTR_EFFECT in kwargs:
            affectAttr = kwargs.get(ATTR_EFFECT)
            effect = [e for e in EFFECT_LIST if e['name'] == affectAttr][0]
            
            if 'mode' in effect:
                mode = effect['mode']

            if 'effect' in effect:
                effectNr = effect['effect']
                
        self._dev.turnOn(brightness=brightness, rgb=rgb, temp=temp, mode=mode, effect=effectNr)
    ...
    def hs_color(self):
        ...
        hs = color_util.color_RGB_to_hs(*self._rgb)
        return hs
